...
# leaping
    # keep adding score if reached threshold
        execute if score @s donpig_abrm.asset.mob.timer_1 matches 40.. run scoreboard players add @s donpig_abrm.asset.mob.timer_1 1
    # manage timer
        execute if predicate ancestral_beats_remake:lib/is_on_ground unless score @s donpig_abrm.asset.mob.timer_1 matches 40.. run scoreboard players add @s donpig_abrm.asset.mob.timer_1 1
    # caution
        execute if score @s donpig_abrm.asset.mob.timer_1 matches 40 run function ancestral_beats_remake:asset/mob/zombie/action/leaping/caution
    ...
    # run
        execute if score @s donpig_abrm.asset.mob.timer_1 matches 60 rotated ~ 0 run function ancestral_beats_remake:asset/mob/zombie/action/leaping/
    # reset
        execute if score @s donpig_abrm.asset.mob.timer_1 matches 80.. run scoreboard players reset @s donpig_abrm.asset.mob.timer_1
