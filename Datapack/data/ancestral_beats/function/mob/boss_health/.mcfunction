...
# calculation
    ...
    scoreboard players operation $damage donpig_ab.temporary += $2048 constant
    execute if entity @s[tag=donpig_ab.asset.mob.boss_invul] run scoreboard players set $damage donpig_ab.temporary 0
    scoreboard players operation @s mob.health -= $damage donpig_ab.temporary
...
# death
    execute if score @s mob.health matches 0 run tag @s add donpig_ab.asset.mob.dead
